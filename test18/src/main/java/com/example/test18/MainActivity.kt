...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // doGetUserList 클래스에 붙였다. doGetUserList
        // 예 doGetUserList(2)
        ...
        userListCall.enqueue(
            ...
            object : Callback<UserListModel>{
                //Callback 인터페이스를 구현한 특정 표현식을 쓸 때, 의무적으로 재정의를 해야하는 함수 2개
                //onResponse : 서버로부터 응답 성공했을 때, 수행하는 함수
                //onFailure : 실패했을 때, 수행하는 함수
                override fun onResponse(
                    call: Call<UserListModel>,
                    response: Response<UserListModel>
                ) {
                    // 정상적으로 서버부터 데이터를 받아왔다.
                    // 실제로 데이터를 받고 나서, response.body 곳에 data가 있음
