...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        requestPermissionLauncher = registerForActivityResult(
            ...
            //넘어온 정보는 해당 앱에 대한 접근을 하기 위한 권한에 대한 승인 정보
            // it : 하나의 매개 변수를 가리킴
            for (entry in it.entries) {
                //android.permission.READ_CONTACTS : 액션 문자열, 상수로 정해져있음
                //외부 저장소, 특정 권한 관련된 문자 상수 여러개
                if(entry.key == "android.permission.READ_CONTACTS" && entry.value) {
                    Log.d("kkang", "granted ok...")
                    //Intent.ACTION_PICK -> 두번재 매개변수 데이터가 연락처, 위도 경도, 웹 주소 등에 따라서
                    //호출하는 앱이 다르다. 연락처 앱, 지도 앱, 브라우저.
                    val intent =
                        Intent(Intent.ACTION_PICK, ContactsContract.CommonDataKinds.Phone.CONTENT_URI)
                    // 후처리의 시작, 현재 앱 호출 -> 연락처 앱 : 데이터 선택 -> 현재 앱으로 돌아옵니다.
        ...
        // 똑같은 패턴, 사진첩, 카메라, 모든 외부 앱의 데이터 연동 부분
            ...
            ActivityResultContracts.StartActivityForResult())
        {
            ...
            if(it.resultCode == RESULT_OK){
                //it.data 이 부분에는 선택된 연락처 정보 하나가 있음
