...
class MainActivity2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 에뮬레이터 실제 물리 경로 : storage -> emulated -> 0 -> Android-> data -> com.example.test16
        // 참고코드 : ch16_provider -> MainActivity
        // 뷰는 재활용, 복사
        // 순서 1
        //gallery request launcher..................
        // 갤러리 앱에서 사진 선택 한 데이터를 여기서 처리합니다.
        ...
        // 앱 별 저장소(우리가 지정한, Pictures에 파일로 저장 작업)
        val requestCameraFileLauncher = registerForActivityResult(
            // 후처리
            ActivityResultContracts.StartActivityForResult()){
            // 원본 사진 크기 그대로 읽거나 처리 작업하면, OOM 사진을 줄여야 한다.

            // 갤러리 설명했음.
            val calRatio = calculateInSampleSize(
                // 저장할 물리파일의 위치를 Uri 타입으로 변경.
        ...
        binding.cameraButton.setOnClickListener {
            ...
            // xml -> 경로 설정
            // DIRECTORY_PICTURES 상수. 즉, 갤러리 위치에 물리 파일 저장.
            ...
            // 암구호 : com.example.ch16_provider.fileprovider 인증이 되면
            ...
            // value 값은 : photoURI : contenxt 자기 자신 객체, 액티비티
            // 암구호 : com.example.ch16_provider.fileprovider
            // 물리 파일 이동
            intent.putExtra(MediaStore.EXTRA_OUTPUT, photoURI)
            // 후처리 작업. 현재 앱에서 -> 카메라 앱, 사진 촬영한 사진 데이터 -> 현재 앱으로 가져오기.
            requestCameraFileLauncher.launch(intent)

        }
    }

    //이미지의 크기를 원본으로 읽으며, OOM(Out Of Memory) 현상으로 문제가 됨.
    // 이미지의 크기를 적당히 조절하는 함수 인데
    // 예를 들어, 현재 뷰의 가로 세로 150dp 150dp
    // 실제 사진의 해상도 : 3000dp, 2000dp -> 반으로 줄여 나감.
    // 1500 1500 -> 750 500 -> 375 250 -> 187.5 125
