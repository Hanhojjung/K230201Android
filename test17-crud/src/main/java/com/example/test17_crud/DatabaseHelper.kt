...
//뷰 객체 선택 방법이 findByID -> 뷰바인딩 형식으로 변경
class DatabaseHelper(context: Context?) : SQLiteOpenHelper
    (context, DATABASE_NAME, null, 1) {
    // 1번 매개변수 context -> this 현재 호출이 된 액티비티를 의미.
    // 2번 매개변수 데이터베이스 파일명
    // -> 예) DATABASE_NAME 상수로 선언해서 사용중
    // 4번째 매개면수, 현재 사용하는 버전을 의미하고 이 버전이 변경이 된다면 ,
    // 아래에 있는 onUpgrade 함수 재호출됨

    override fun onCreate(db: SQLiteDatabase) {
        // 앱이 설치 후, 해당 클래스가 최초에 실행이 되면,
    ...
    // 생성된 물리 경로 주소.
    // -> 에뮬레이터 탐색기 -> data -> data 패키지명:com.example.test17_crud -> database, 파일명

    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        // SQLiteOpenHelper
        //    (context, DATABASE_NAME, null, 1)
        // 4번째 매개변수가 변경이 될 때마다, 실행되는 함수.
        // 예를 들어 버전 -> 2로 변경 시 아래 함수가 실행됨.
    ...
    fun insertData(name: String?, phone: String?, address: String?): Boolean {
        // DB 사용 시 쓰기, 수정, 삭제 -> writableDatabase 사용
        val db = this.writableDatabase
        // execSQL -> 대신에 ContentValues()를 이용하면 SQL 문장없이,
        // 바로 메서드에 값만 인자로 넣어서 이용하면 쉽게 insert를 구현가능.
